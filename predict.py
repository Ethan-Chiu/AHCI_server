        ...
        for path, im, im0s, vid_cap, s in dataset:
            ...
            for i, det in enumerate(pred):  # per image
                ...
                if len(det):
                    ...
                    masks = process_mask(proto[-1][i], det[:, 6:], det[:, :4], im.shape[2:], upsample=True)
                    ormask = np.logical_or.reduce(masks, axis=0)[..., np.newaxis]
                    output = np.concatenate((im0, ormask), axis=-1)
